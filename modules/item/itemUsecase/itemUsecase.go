...
func (u *itemUsecase) FindManyItems(pctx context.Context, basePaginateUrl string, req *item.ItemSearchReq) (*models.PaginateRes, error) {
	...
	if req.Title != "" {
		findItemsFilter = append(findItemsFilter, bson.E{Key: "title", Value: primitive.Regex{Pattern: req.Title, Options: "i"}})
		countItemsFilter = append(countItemsFilter, bson.E{Key: "title", Value: primitive.Regex{Pattern: req.Title, Options: "i"}})
	}
